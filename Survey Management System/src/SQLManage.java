...
public class SQLManage {
    ...
    public void newQuestion(String code, String question, String op1, String op2, String op3, String op4)
            throws SQLException {
        String str = "INSERT INTO questions values (" + code + ", " + question + ", " + op1 + ", " + op2 + ", " + op3
                + ", " + op4 + ",)";
        Statement stm = con.createStatement();
        stm.executeUpdate(str);
    }
    ...
    public int getCount(String surveycode, int qno, int op) throws SQLException {
        String str = "SELECT count(opno) FROM surveyquestions WHERE surveycode = '" + surveycode + "' AND qno = "
                + (qno + 1) + " AND opno = " + op;
        Statement stm = con.createStatement();
        ResultSet rst = stm.executeQuery(str);
        if (rst.next())
